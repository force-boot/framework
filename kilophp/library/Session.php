...
class Session
{
    ...
    public static function init(array $config = [])
    {
        ...
        if (!empty($config['type'])) {
            // 读取session驱动
            $class = false !== strpos($config['type'], '\\') ? $config['type'] : '\\kilophp\\session\\driver\\' . ucwords($config['type']);
            // 检查驱动类
            if (!class_exists($class) || !session_set_save_handler(new $class($config))) {
                throw new Exception(lang('no_session_handler', $config['type']));
            }
        }
        if ($isDoStart) {
            session_start();
     ...
     * @throws Exception
     */
    public static function set(string $name, $value = '', string $prefix = null)
    {
        empty(self::$init) && self::boot();
     ...
     * @throws Exception
     */
    public static function get(string $name = '', string $prefix = null)
    {
        empty(self::$init) && self::boot();
        $prefix = !is_null($prefix) ? $prefix : self::$prefix;
        if ('' == $name) {
     ...
     * @throws Exception
     */
    public static function pull(string $name, string $prefix = null)
    {
     ...
     * @throws Exception
     */
    public static function flash(string $name, $value)
    {
        self::set($name, $value);
     ...
     * @throws Exception
     */
    public static function delete(string $name, string $prefix = null)
    {
        empty(self::$init) && self::boot();
        $prefix = !is_null($prefix) ? $prefix : self::$prefix;
        if (is_array($name)) {
     ...
     * @throws Exception
     */
    public static function clear(string $prefix = null)
    {
        empty(self::$init) && self::boot();
        $prefix = !is_null($prefix) ? $prefix : self::$prefix;
        if ($prefix) {
     ...
     * @throws Exception
     */
    public static function has(string $name, string $prefix = null): bool
    {
        empty(self::$init) && self::boot();
        $prefix = !is_null($prefix) ? $prefix : self::$prefix;
        if (strpos($name, '.')) {
